...
const Button: React.FC<React.PropsWithChildren<{ onClick?: () => void; href?: string; variant?: "solid" | "ghost"; className?: string; type?: "button" | "submit" }>> = ({ onClick, href, children, variant = "solid", className = "", type = "button" }) => {
  const base = "inline-flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-medium transition-transform active:scale-[0.98]";
  const styles = variant === "ghost"
    ? "border border-black/10 bg-transparent text-black hover:bg-black hover:text-white"
    : "bg-black text-white hover:opacity-90";
...
// -----------------------------
// ContactForm with Formspree
// -----------------------------
function ContactForm() {
  const [state, handleSubmit] = useForm("mnngvegy"); // replace with your Formspree ID
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <input
          ...
          className="rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-black"
        />
        <input
          name="company"
...
// -----------------------------
const ConsultingLandingPage: React.FC = () => {
  const BRAND = "Ashbourne & Wells";
  const TAGLINE = "Clarity. Execution. Results.";
  const LEDE = "We help businesses look sharp and scale with confidence—without adding chaos.";

  const SERVICES = [
    { icon: <Briefcase className="h-6 w-6" />, title: "Strategy & Roadmapping", text: "Prioritized, outcome-driven plans that align stakeholders and unlock momentum." },
    { icon: <Layers className="h-6 w-6" />, title: "Delivery & QA Systems", text: "Lightweight processes, CI/CD, and test harnesses that raise quality and speed." },
    { icon: <LineChart className="h-6 w-6" />, title: "Analytics & Optimization", text: "Instrumentation and feedback loops to measure what matters and iterate smarter." },
  ];

  const ABOUT = {
    heading: "Pragmatic partners, not slide-makers",
    body: `We're builders at heart. We partner with founders and product teams to solve real problems, ship real features, and leave you with durable systems—not dependency. Our approach is transparent, test-driven, and tailored to your constraints.`,
    bullets: ["Senior hands-on expertise", "Clear comms, weekly outcomes", "Right-sized process, zero theatre"],
  } as const;
    ...
    <div className="min-h-screen bg-white text-black">
      ...
      <header className="sticky top-0 z-40 border-b border-black/10 bg-white/80 backdrop-blur">
        <Container className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden gap-6 sm:flex">
            {NAV.map((item) => (
              <button key={item.id} onClick={() => scrollToId(item.id)} className="text-sm text-black/70 hover:text-black">
                {item.label}
              </button>
            ))}
          </nav>
          <div className="flex items-center gap-3">
            <Button href="#contact" onClick={() => scrollToId("contact")} className="hidden sm:inline-flex">
              Get in touch <ArrowRight className="h-4 w-4" />
            </Button>
            <button className="sm:hidden" aria-label="Open menu" onClick={() => scrollToId("contact")}>
              <Mail className="h-5 w-5" />
      ...
      <Section id="hero">
        <Container>
          <h1 className="text-4xl font-bold">{BRAND}: {TAGLINE}</h1>
          <p className="mt-4 max-w-2xl text-black/70">{LEDE}</p>
        </Container>
      </Section>
      ...
      <Section id="about">
        <Container>
          ...
          <p className="mt-2 text-sm text-black/70">{ABOUT.body}</p>
          <ul className="mt-4">
            {ABOUT.bullets.map((b) => (
      ...
      {/* Footer */}
      <footer className="border-t border-black/10 bg-white py-10">
        <Container className="flex justify-between text-xs text-black/60">
